...
class EnumerablesTest < Minitest::Test
  ...
  def test_find_waldo
    ...
    actual = words.find {|word| word == "waldo"}
  ...
  def test_array_with_the_most_items
    ...
    actual = arrays.max_by {|ele| ele.count}
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    # initial value is 1
    actual = numbers.reduce(1) {|acc, num| acc * num}
  ...
  def test_subtract_smallest_values_from_100
    ...
    actual = elements.reduce(100) do |acc, num|
      acc -= num.min
    end
  ...
  def test_one_double_digit_number
    ...
    actual = numbers.one? {|num| num.digits.count == 2}
    assert actual
  end
  ...
  def test_count_round_prices
    ...
    actual = prices.count {|num| num % 1 == 0}
    assert_equal 2, actual
  end
