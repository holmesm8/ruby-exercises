...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_vowels
    ...
    letters.each do |letter|
      remaining << letter unless letter.include?("e") or ("a") or ("o") or ("i") or ("u")
    end
  ...
  def test_remove_words_containing_e
    ...
    selected = []
    words.each do |word|
      selected << word unless word.inlcude?("e") == true
    end
  ...
  def test_remove_dinosaurs
    ...
    animals.each do |animal|
      notasaurus << animal unless animal[-4..-1] == "urus"
    end
  ...
  def test_remove_numbers
    ...
    elements.each do |ele|
      not_numbers << ele unless ele.class != String
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |ele|
      not_numbers << ele unless ele.class == Float
    end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |ele|
      remaining << ele unless ele.class == Array
    end
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |ele|
      remaining << ele unless ele.class == Hash
    end
