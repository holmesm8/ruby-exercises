...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    result = numbers.reduce(1) do |multi, num|
      multi * num
    end
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |acc, word|
  ...
  def test_divide_560_by_a_bunch_of_numbers
    numbers = [2, 2, 2, 5, 7]
    result = numbers.reduce(560) do |acc, num|
      acc /= num
    end
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) do |acc, ele|
      acc -= ele.min
    end
  ...
  def test_add_all_the_second_values_together
    ...
    result = elements.reduce(0) do |acc, ele|
      acc += ele[1]
    end
