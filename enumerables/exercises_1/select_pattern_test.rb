...
class SelectPatternTest < Minitest::Test
  ...
  def test_pick_dinosaurs
    ...
    dinosaurs = []
    animals.each {|animal| dinosaurs << animal if animal.include?("saurus")}
  ...
  def test_pick_floats
    ...
    floats = []
    numbers.each {|num| floats << num if num.class == Float}
  ...
  def test_pick_arrays
    ...
    arrays = []
    elements.each {|ele| arrays << ele if ele.class == Array}
  ...
  def test_pick_hashes
    ...
    hashes = []
    elements.each {|ele| hashes << ele if ele.class == Hash}
