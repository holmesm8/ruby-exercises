...
class CountPatternTest < Minitest::Test
  ...
  def test_6
    ...
    ages.each do |key, value|
      multiple_of_three += 1 if value % 3 == 0
    end
    assert_equal 3, multiple_of_three
  end
  ...
  def test_7
    ...
    family.each {|fam| names_with_three_letters += 1 if fam.length == 3}
  ...
  def test_8
    ...
    family.each do |key, value|
      names_with_three_letters += 1 if value.length == 3
    end
    assert_equal 2, names_with_three_letters
  end
  ...
  def test_9
    ...
    whole_numbers = 0
    prices.each {|price| whole_numbers += 1 if price % 1 == 0}
