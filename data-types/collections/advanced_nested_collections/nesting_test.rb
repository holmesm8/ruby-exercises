...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_all_ingredients_across_all_restaurants
    #=======================
    ingredients = stores.map do |store, data|
      data[:dishes].map do |stuff|
        stuff[:ingredients]
      end
    end.flatten
    #=======================
  ...
  def test_full_menu_price_for_olive_garden
    #=======================
    full_menu_price = stores[:olive_garden][:dishes].reduce(0) do |collection, price|
      collection += price[:price]
    # testy = stores[:olive_garden][:dishes].map do |data|
    #   full_menu_price += data[:price]
    # end
  end
    #=======================
  ...
  def test_full_menu_for_olive_garden
    #=======================
    # olive_garden_menu = stores[:olive_garden][:dishes].reduce({}) do |hash, key|
    olive_garden_menu = {}
  ...
  def test_menu_accross_all_restaurants
    #=======================
     # full_menu ={}
     # stores.each do |store, dish|
     #   dish[:dishes].each do |dish|
     #     full_menu[dish[:name]] = dish
     #   end
     # end

    full_menu = stores.reduce({}) do |keys, data|
      dishes = data[1][:dishes]
      dishes.each do |dish|
        keys[dish[:name]] = dish
      end
      keys
